...
public class Tree<E> {
  ...
  public static int findMax(Tree<Integer> tree) {
      if (tree.root == null) {
          throw new IllegalArgumentException("Cannot find max in an empty tree!");
      } else {
  ...
  public static int findMax(Node<Integer> node) {
      // where might the max be?
      // might be node.value
      int ans = node.value;
      // might be in the left subtree
      if (node.left != null) {
          // go find the answer on the left
          // save whichever of ans, max on left is bigger
          ans = Math.max(ans, findMax(node.left));
      }
      if(node.right != null) {
          ans = Math.max(ans, findMax(node.right));
      }
